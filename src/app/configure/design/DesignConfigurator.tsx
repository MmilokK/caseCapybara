...
const {mutate: saveConfig, isPending} = useMutation({
    ...
    onError: () => {
        toast({title: "Что-то пошло не так", description: "Произошла ошибка. Попробуйте еще раз", variant: "destructive"})
    ...
    const [renderedDimension, setRenderedDimension] = useState({width: imageDimensions.width/4, height: imageDimensions.height/4})
    ...
    async function saveConfiguration() {
        try {
            ...
            toast({title: "Что-то пошло не так", description: "Есть проблема при сохранении, поажлуйста, попробуйте еще раз.", variant: "destructive"})
        }
    }
    ...
    return <div className="relative mt-20 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-20">
        <div ref = {containerRef} className="relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
            <div className="relative w-60 bg-opacity-50 pointer-events-none aspect-[896/1831]">
                ...
                <div className="absolute z-40 inset-0 left-[3px] top-px right-[3px] bottom-rx rounded-[32px] shadow-[0_0_0_99999px_rgba(229,231,235,0.6)]" />
            ...
            className="absolute x-20 border-[3px] border-primary" lockAspectRatio resizeHandleComponent={{
        ...
        <div className='h-[37.5rem] w-full col-span-full lg:col-span-1 flex flex-col bg-white'>
            <ScrollArea className="relative flex-1 overflow-auto">
                ...
                <div className="px-8 pb-12 pt-8">
                    <h2 className="tracking-tight font-bold text-3xl">Кастомизируйте свой чехол</h2>
                    ...
                    <div className="relative mt-4 h-full flex flex-col justify-between">
                        ...
                        {[MATERIALS, FINISHES].map(({name, options: selectableOptions})=>(
                            <RadioGroup key={name} value={options[name]} onChange={(val)=>{
                                ...
                                <Label>
                                    {name === "material"? "Материал" : "Покрытие"}
                                </Label>
                                        ...
                                        <RadioGroup.Option key={option.value} value={option} className={({active, checked})=> cn("relative block cursor-pointer rounded-lg bg-white px-6 py-4 shadow-sm border-2 border-zinc-200 focus:outline-none ring-0 focus:ring-0 outline-none sm:flex sm:justify-between", {'border-primary': active || checked})}>
                                            <span className="flex items-center">
                                                <span className="flex flex-col text-sm">
                                                    <RadioGroup.Label className="" as="span">
                                                        {option.label}
                                                    </RadioGroup.Label>
                                            ...
                                            <RadioGroup.Description as="span" className="font-medium text-gray-900">
                                                <span>
            ...
            <div className="w-full px-8 h-16 bg-white">
                ...
                <div className="w-full h-full flex justify-end items-center">
                    <div className=" w-full flex gap-6 items-center">
                        ...
                        <Button 
                        isLoading = {isPending} disabled = {isPending} loadingText="Сохранение"
                        onClick={()=>saveConfig({
                            configId, 
